...
export class BusinessleadsComponent implements OnInit {
  ...
  verfiy() {
    if (
      JSON.stringify(this.datas.value.whatsapp).length >= 1 &&
      JSON.stringify(this.datas.value.mobile).length >= 1 &&
      this.datas.value.nature.length >= 1 &&
      this.datas.value.town.length >= 1 &&
      this.datas.value.name.length >= 1 &&
      this.datas.value.company.length >= 1 &&
      this.datas.value.textArea.length >= 1 &&
      this.datas.value.place.length >= 1
    ) {
      // console.log("hello");
      this.color = 'primary';
    }
  ...
  submit() {
    ...
    if (
      (JSON.stringify(this.datas.value.mobile).length == 9 ||
        JSON.stringify(this.datas.value.mobile).length == 10) &&
      (JSON.stringify(this.datas.value.whatsapp).length == 9 ||
        JSON.stringify(this.datas.value.whatsapp).length == 10)
    ) {
      var userId = localStorage.getItem('userid');
      var data = {
        userid: userId,
        name: this.datas.value.name,
        company: this.datas.value.company,
        place: this.datas.value.place,
        mobileNo: JSON.stringify(this.datas.value.mobile),
        whatsappNo: JSON.stringify(this.datas.value.whatsapp),
        natureOfBusiness: this.datas.value.nature,
        businessDetails: this.datas.value.textArea,
      };
      const url = 'https://feedback.apmkingstrack.com/feedback/api/insert/lead';
      this.ajaxService.ajaxPutMethod(url, data).subscribe((res) => {
        // console.log(res);
        if (res.message == 'Persisted') {
          this.router.navigateByUrl('/home');
          this.commonService.presentToast(
            'Your details are successfully registered, Thank you'
          );
        } else {
          this.router.navigateByUrl('/businessleads');
        }
      });
    } else {
      this.commonService.presentToast(
        'please check your mobile number OR whatapp number'
      );
    }
  ...
  ngOnInit() {
    ...
    if (localStorage.getItem('language') === 'tan') {
      ...
      this.newBuss =
        this.language['tanzania']['New Bussiness Opportunity Details.'];
      ...
      this.newBuss =
        this.language['english']['New Bussiness Opportunity Details.'];
